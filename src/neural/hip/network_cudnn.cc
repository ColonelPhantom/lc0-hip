...
template <typename DataType>
class HipdnnNetworkComputation : public NetworkComputation {
 public:
  HipdnnNetworkComputation(HipdnnNetwork<DataType>* network, bool wdl,
                          bool moves_left);
  ~HipdnnNetworkComputation();
 ...
 private:
  ...
  HipdnnNetwork<DataType>* network_;
};

template <typename DataType>
class HipdnnNetwork : public Network {
 public:
  HipdnnNetwork(const WeightsFile& file, const OptionsDict& options)
    ...
    ReportHIPDNNErrors(hipdnnCreate(&cudnn_));
    ...
    #if FALSE
    // AMD does not have tensor cores
    if (hasTensorCores)
    ...
    ReportHIPDNNErrors(hipdnnSetFilter4dDescriptor(
        wDesc, layout, datatype, maxChannels, maxChannels, 3, 3));

    ReportHIPDNNErrors(hipdnnSetTensor4dDescriptor(
    ...
    ReportHIPDNNErrors(hipdnnGetConvolutionForwardWorkspaceSize(
  ...
  std::unique_ptr<NetworkComputation> NewComputation() override {
    ...
    ReportHIPErrors(hipSetDevice(gpu_id_));
    return std::make_unique<HipdnnNetworkComputation<DataType>>(this, wdl_,
                                                               moves_left_);
  }
 ...
 private:
  ...
  void showInfo() const {
    ...
    CERR << "Hipdnn version: " << major << "." << minor << "." << pl;
...
template <typename DataType>
HipdnnNetworkComputation<DataType>::HipdnnNetworkComputation(
    HipdnnNetwork<DataType>* network, bool wdl, bool moves_left)
...
template <typename DataType>
void HipdnnNetworkComputation<DataType>::ComputeBlocking() {
  network_->forwardEval(inputs_outputs_.get(), GetBatchSize());
}

template <typename DataType>
std::unique_ptr<Network> MakeHipdnnNetwork(const std::optional<WeightsFile>& w,
  ...
  return std::make_unique<HipdnnNetwork<DataType>>(weights, options);
}

std::unique_ptr<Network> MakeHipdnnNetworkAuto(
  ...
  if (deviceProp.major >= 7 ||
    ...
    return MakeHipdnnNetwork<half>(weights, options);
  }
  CERR << "Switching to [cudnn]...";
  return MakeHipdnnNetwork<float>(weights, options);
}

REGISTER_NETWORK("hipdnn-auto", MakeHipdnnNetworkAuto, 120)
REGISTER_NETWORK("hipdnn", MakeHipdnnNetwork<float>, 110)
REGISTER_NETWORK("hipdnn-fp16", MakeHipdnnNetwork<half>, 105)
