...
void SELayer<half>::Eval(int N, half* output, const half* input,
  ...
  if (!se_done) {
    ...
    ReportCUBLASErrors(hipblasHgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, numFc1Out_,
                                   N, C, (hipblasHalf*)&alpha, (hipblasHalf*)w1_, C, (hipblasHalf*)op2, C, (hipblasHalf*)&beta, (hipblasHalf*)op1,
                                   numFc1Out_));
    addVectors(op1, b1_, op1, numFc1Out_ * N, numFc1Out_, numFc1Out_ * N, true,
               false, false, stream);
    ...
    ReportCUBLASErrors(hipblasHgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, 2 * C, N,
                                   numFc1Out_, (hipblasHalf*)&alpha, (hipblasHalf*)w2_, numFc1Out_, (hipblasHalf*)op1,
                                   numFc1Out_, (hipblasHalf*)&beta, (hipblasHalf*)op2, 2 * C));
    addVectors(op2, b2_, op2, 2 * C * N, 2 * C, 2 * C * N, false, false, false, stream);
...
void FCLayer<half>::Eval(int N, half* output_tensor, const half* input_tensor,
  ...
  ReportCUBLASErrors(hipblasHgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, num_outputs,
                                 N, num_inputs, (hipblasHalf*)&alpha, (hipblasHalf*)weights_, num_inputs,
                                 (hipblasHalf*)input_tensor, num_inputs, (hipblasHalf*)&beta, (hipblasHalf*)output_tensor,
                                 num_outputs));
