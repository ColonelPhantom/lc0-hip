...
template <typename T>
void addBias_NCHW(T* c, T* a, T* b, int N, int C, int H, int W, bool relu, hipStream_t stream) {
  int size = N * C * H * W;
  const int kBlockSize = 256;
  int blocks = DivUp(size, kBlockSize);
...
void fp32NCHWtofp16NHWC(half* output_tensor, float* input_tensor, int Nin,
  ...
  hipLaunchKernelGGL(fp32NCHWtofp16NHWC_kernel, dim3(blocks), dim3(blockSize), 0, 0, output_tensor, input_tensor,
...
void copyTypeConverted(DstType* op, SrcType* ip, int N, hipStream_t stream) {
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(copyTypeConverted_kernel), dim3(blocks), dim3(kBlockSize), 0, stream, op, ip, N);
}
...
#pragma unroll
  for (int offset = 1; offset < 32; offset *= 2) {
    S += __shfl_down(0xFFFFFFFF, S, offset);
  }
...
void globalAvgPool(int N, int C, T* output, const T* input,
  ...
  if (nhwc) {
    ...
    hipLaunchKernelGGL(globalAvgPool_kernel_NHWC_fp16, dim3(N), dim3(C), 0, 0, (half*)output, (half*)input,
                                             (half*)prevLayerBias,
                                             N * C * kPlaneSize, N * C);
  } else {
...
void globalScale(int N, int C, T* output, const T* input, const T* scaleBias,
  ...
  if (nhwc) {
    assert(fp16);
    hipLaunchKernelGGL(globalScale_kernel_fp16_nhwc, dim3(kBlocks), dim3(kBlockSize), 0, 0, (half*)output, (half*)input, (half*)scaleBias, (half*)prevLayerBias,
        N * C * 8 * 8, C, 8 * 8 * C);
  } else {
...
void OutputInputTransform(int N, int C, int se_K, T* output, const T* input,
  ...
  if (C > kMaxResBlockFusingChannels) {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(OutputTransform_SE_relu_InputTransform_kernel<float, use_se, relu, use_bias,
...
template void addVectors<float>(float* c, float* a, float* b, int size,
                                ...
                                bool use_sigmoid, hipStream_t stream);
...
template void addBias_NCHW<float>(float* c, float* a, float* b, int N, int C,
                                  int H, int W, bool relu, hipStream_t stream);

template void addBias_NCHW<half>(half* c, half* a, half* b, int N, int C, int H,
                                 int W, bool relu, hipStream_t stream);
